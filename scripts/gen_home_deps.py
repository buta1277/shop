...
def load_index_sections():
    """从 templates/index.json 读取首页用到的 section 类型"""
    ...
    for sec_id, sec in sections.items():
        stype = sec.get("type")
        if stype:
            used.add(stype if stype.endswith(".liquid") else f"{stype}.liquid")
    return sorted(used)
    ...
    for sec in used_sections:
        ...
        for sn in renders:
            ...
            if os.path.exists(sn_path):
                ...
                nested |= scan_renders(sn_text)

        lines.append(f"- snippets: {', '.join(sorted(renders)) if renders else '(无)'}")
        if nested:
            lines.append(f"  - nested snippets: {', '.join(sorted(nested))}")
